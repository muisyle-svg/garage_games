/*
  ...
  Game:
    ...
    - Hold the master button for 3 seconds to begin discovery.
...
constexpr uint32_t MASTER_HOLD_MS = 3000;
...
void updateHoldIndicator(uint32_t now) {
  ...
  // green as the three-second hold completes.
...
void updateMasterButton() {
  ...
  if (reading) {
    ...
    bool completedIdlePress = gameState == IDLE && masterHoldActive &&
                              !masterStartTriggered;
    uint32_t pressDurationMs = now - masterHoldStartedMs;
    masterHoldActive = false;
    masterStartArmed = true;
    if (gameState == IDLE) {
      stopLEDPattern();
      if (completedIdlePress) recordQuickTap(now, pressDurationMs);
    } else {
      quickTapCount = 0;
      lastQuickTapMs = 0;
    }
...
void setup() {
  ...
  showText4("RDY ");
  bootReadyDisplayed = true;
  Serial.printf("[OK] Master ready; ESP-NOW channel %u; hold button 3 seconds.\n", ESPNOW_CHANNEL);
}
